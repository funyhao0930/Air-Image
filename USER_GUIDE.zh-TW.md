...
1. 將指尖放在預定的 `1` 鍵左上角，按 `Space` 開始取樣，保持不動直到畫面顯示已記錄。
2. 將指尖移到 `3` 鍵右側位置，按 `Space` 開始取樣，保持不動直到畫面顯示已記錄。
3. 將指尖移到 `0` 鍵下方位置，按 `Space` 開始取樣，保持不動直到畫面顯示已記錄。

每個位置預設收集 18 筆有效 XYZ，使用中位數與 MAD 排除離群值；取樣時畫面會顯示進度，完成後列出 XYZ 散布。第 2、3 點不必精確落在鍵面中心，但應與第 1 點保持明顯距離；預設至少約 80 mm，且右方與下方兩個方向不可接近同一直線。第三點記錄後，狀態會提示按 `Enter` 完成校正。
...
- 失敗時會清除三點並提示從 `1` 鍵左上角重新取樣，避免卡在無法覆蓋的 3/3 狀態。
