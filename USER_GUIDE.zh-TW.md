...
1. 讓電腦看見 Gemini 2。
2. 從正確的輸出資料夾啟動程式。
3. 用 `C`、表面掃描和七個校正點告訴程式「虛擬鍵盤在哪裡、大小是多少」。
4. 用食指靠近平面，讓程式產生按鍵事件。
...
所有點都要指向邊界或角點，不要指向數字字形中心。校正前的表面掃描會先建立目標平面，因此 P1～P7 不會直接把指尖當成表面位置；前三點仍要有明顯距離，預設 P2、P3 距離 P1 至少約 80 mm。

### 開始校正

1. 先確認畫面顯示 `追蹤：本幀有效`；看到 `XYZ` 代表指尖深度也正常。
2. 按 `C`。
3. 將食指輕碰或靠近目標表面，沿預計的鍵盤範圍慢慢移動。
4. 按一次 `Space` 開始表面掃描；掃描會自動完成，不要立刻按 `Enter`。
...
掃描完成後，畫面會顯示「將手指移到 1 鍵左上角」。這時才移到 P1，並開始七點取樣。掃描時，程式會讀取指尖周邊未被遮住的目標深度；紙張、板面或螢幕周邊必須在相機深度畫面中可見。
...
這一步最容易做錯：P3 不是鍵盤右下角，也不是 `0` 鍵右下角。成功後，畫面會要求前往 P4。
...
如果按 `Enter` 後失敗，程式會清除七個點但保留已驗證的表面掃描，請按畫面提示從 P1 重新開始。只有相機、紙張或鍵盤所在表面已移動時，才按 `C` 重新掃描。
...
| 按鍵 | 功能 | 什麼時候使用 |
| --- | --- | --- |
| `C` | 清除目前校正，準備表面掃描與七點校正。 | 相機、紙張或虛擬鍵盤位置改變時。 |
| `S` | 開啟／隱藏「參數設定」視窗。 | 調整參數或查看距離預覽時。 |
| `Space` | 開始表面掃描或目前校正點的連續取樣。 | C 後先掃描；掃描完成後用於七點取樣。 |
...
2. 避免純黑、透明、鏡面或強反光表面。
3. 保持指尖不動。
4. 表面掃描時，確認 `追蹤：本幀有效` 且指尖周圍看得到目標表面後再按 `Space`；七點取樣時，依畫面的「等待有效指尖位置與表面射線」提示調整手勢。
...
表面基準會被保留，從 P1 重新做即可；只有目標位置變動時才按 `C` 重掃。畫面會指出最先不符的項目。重做時逐項檢查：
...
```text
C → 沿鍵盤區域移動 → Space 表面掃描 → 自動完成 → P1 Space → P2 Space → P3 Space → P4 Space → P5 Space → P6 Space → P7 Space → Enter
```
...
- 不包含 ASKA3D 或雙手操作。
- 校正表面與鍵盤幾何只保留在本次執行期間，重新啟動後要重新掃描並做七點校正。
