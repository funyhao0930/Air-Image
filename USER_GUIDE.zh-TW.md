...
本指南適合第一次使用本 MVP 原型的使用者，說明如何從啟動程式開始，完成七點校正並操作空中數字鍵盤。
...
- 視窗右下角的 1–9、0 數字鍵盤。
- 主程式 FPS、相機實際 FPS／深度模式／深度精度／D2C、追蹤、指尖座標、校正、觸控與最近事件等資訊。
...
| 畫面項目 | 意義 |
| --- | --- |
| `主程式 FPS／相機` | 主迴圈更新速度，以及相機實際使用的 FPS、深度模式與深度精度。 |
| `對齊` | 顯示硬體 D2C 或軟體 D2C；兩者都是 RGB 與深度的對齊方式。 |
| `追蹤：本幀有效` | 本幀有真實且有限的 MediaPipe 指尖觀測，可以參與校正與觸控判定。 |
| `追蹤：未取得本幀觀測` | 可能短暫保留灰色指尖位置供觀看，但不會用它產生按鍵事件。 |
| `追蹤：無法使用` | MediaPipe bridge 或手部模型未成功載入。 |
| `指尖像素（濾波）` | One Euro Filter 後的指尖像素位置；「僅保留顯示」不具事件資格。 |
| `XYZ` | 指尖在相機座標中的三維位置，單位是 mm。 |
| `平面 UV` | 指尖投影到校正平面後的位置，單位是 mm。 |
| `距離` | 同時顯示 raw 與 filtered 的平面距離，單位是 mm；觸控使用 filtered 值。 |
...
在主程式的 OpenCV 視窗按 `S`，會開啟「參數設定」視窗。視窗分成觸控判定、深度與校正、觸控距離預覽及鍵盤幾何四區；可調參數都有滑桿與可精確輸入的數值框，右側預覽會顯示目前指尖距離與觸控狀態，鍵盤幾何區會說明尺寸由七點校正取得。

可調整的項目包含：

- 觸控門檻、離開門檻、最低接近速度與追蹤逾時。
- 相機深度工作模式、深度精度與 FPS；三個欄位都是下拉選單，只顯示目前 Gemini 2 由 Orbbec SDK 回報可用的值，儲存後需重新啟動程式才會套用。
- 深度取樣半徑與校正點最小距離。
- 邊界遲滯；按鍵寬度、高度、水平間距與垂直間距由七點校正自動取得，不在此視窗輸入。

RGB 防閃爍、SDK 濾波器、One Euro、深度跳值、校正取樣與 2 mm 邊界遲滯目前只在 `config/default.yaml` 設定。程式啟動時也會向 SDK 查詢 RGB 防閃爍頻率與 Temporal／Spatial／Hole Filling 是否可用；指定了裝置不支援的項目時會保留目前裝置設定或安全停用，並在 PowerShell 顯示警告。
...
校正用來設定虛擬鍵盤的起點、右方方向、下方方向、整體寬高、按鍵寬高與按鍵間距。每次重新啟動程式，或相機／操作平面位置改變後，都應重新校正。
...
`P3` 必須放在 `0` 鍵正下方的底邊中央，不是紙張右下角；它是整張鍵盤的下邊界參考點。

這些點都是白紙格線上的按鍵邊界或角點，不是數字中心。依下列順序操作；每一點都先讓食指穩定，再按一次 `Space`，取樣期間保持指尖不動，直到畫面顯示已記錄：

1. 將指尖放在 `1` 鍵左上角，按 `Space`。
2. 將指尖放在 `3` 鍵右上角，按 `Space`。
3. 將指尖放在 `0` 鍵正下方的鍵盤下邊界，按 `Space`。
4. 將指尖放在 `1` 鍵右上角，按 `Space`。
5. 將指尖放在 `2` 鍵右上角，按 `Space`。
6. 將指尖放在 `1` 鍵左下角，按 `Space`。
7. 將指尖放在 `4` 鍵左下角，按 `Space`。

每個位置預設收集 18 筆有效 XYZ，使用中位數與 MAD 排除離群值；取樣時畫面會顯示進度，完成後列出 XYZ 散布。前 3 點應與第 1 點保持明顯距離；預設至少約 80 mm，且右方與下方兩個方向不可接近同一直線。第 4～7 點用來量測按鍵寬、高與間距，不套用 80 mm 的最小距離。

若取樣時追蹤逾時、相機影像無效、散布過大或有效樣本不足，該位置不會被記錄；依畫面提示讓指尖穩定後再按一次 `Space`。

### 3. 解算平面

確認畫面顯示已記錄 7/7 後按 `Enter`：

- 成功時顯示鍵盤總寬高、按鍵寬高與水平／垂直間距，校正狀態變成完成，即可操作。
- 失敗時會清除七點並提示從 `1` 鍵左上角重新取樣，避免卡在無法覆蓋的 7/7 狀態。
- 尺寸只保留在本次執行期間；程式重新啟動後必須重新校正。
...
| 按鍵 | 功能 |
| --- | --- |
| `C` | 清除目前校正，開始重新指定七個鍵盤邊界點。 |
| `S` | 開啟或關閉參數設定視窗。 |
| `Space` | 在校正進行中開始收集目前位置的連續樣本。其他時機按下不會擷取校正點。 |
| `Enter` | 在校正進行中，以七個已擷取點解算平面與鍵盤尺寸。 |
...
按 `R` 後重新校正。第 2 點應明確位於第 1 點右方，第 3 點應位於 `0` 鍵正下方；前 3 點要分開足夠距離，且不可幾乎排成一直線。第 4～7 點要準確放在指定按鍵的邊界角點，讓程式量出按鍵本體與間距。
