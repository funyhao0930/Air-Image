...
Windows 原生 C++ 原型：Orbbec Gemini 2 RGB-D、MediaPipe Hand Landmarker、表面掃描加七點鍵盤幾何校正、數字鍵盤與單次觸控事件。
...
- 校正前會從指尖周邊深度掃描目標表面；七個校正位置再以像素射線與表面交點收集 18 筆樣本，避免指尖高度差混入鍵盤幾何。
...
- `C`：開始校正並準備表面掃描。
- `S`：開啟或關閉參數設定視窗。
- `Space`：先開始表面掃描；完成後依序取樣 7 個鍵盤邊界點，讓程式自動取得鍵盤尺寸。
- `Enter`：解算平面與鍵盤幾何。
- `R`：重設校正與觸控狀態。
- `Q`／`Esc`：離開。
...
按 `S` 可調整相機深度工作模式、深度精度、FPS、深度取樣、觸控門檻、追蹤逾時、邊界遲滯與校正點最小距離。鍵盤尺寸與間距不在設定視窗中編輯，而是在每次 `C` 校正時先掃描實體平面，再由 7 個實體邊界點自動計算。深度工作模式、精度與 FPS 都由目前連線的 Gemini 2 實際能力清單建立；YAML 若原本填了不在清單內的值，設定視窗不會把該值重新加入下拉選單。互動參數會立即更新；三個相機參數會寫回 YAML，重新啟動程式後套用到 Gemini 2。其餘 One Euro、深度歷史、校正取樣與邊界遲滯等進階欄位仍保留於 YAML；目前校正幾何只在本次執行期間有效。
