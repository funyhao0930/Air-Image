...
按 `S` 可調整相機深度工作模式、深度精度、FPS、深度取樣、觸控門檻、追蹤逾時、鍵盤尺寸／間距與校正點最小距離。深度工作模式、精度與 FPS 都由目前連線的 Gemini 2 實際能力清單建立；YAML 若原本填了不在清單內的值，設定視窗不會把該值重新加入下拉選單。互動參數會立即更新；三個相機參數會寫回 YAML，重新啟動程式後套用到 Gemini 2。其餘 One Euro、深度歷史、校正取樣與邊界遲滯等進階欄位仍保留於 YAML；目前校正平面不會被清除。
