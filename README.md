...
- 校正前會從指尖周邊掃描目標表面。取樣範圍是半徑 `surface_scan_radius_px` 的圓盤，必須涵蓋鍵盤的相當比例：若只取樣一條細帶，平面可以完美貼合它，但法向量能繞著細帶自由旋轉，而殘差與內點數完全反映不出這個問題。
- 手部像素以關鍵點鄰域排除，不是靠深度門檻——手掌貼在目標上時與目標幾乎共面，深度分不開兩者。另以 `finger_clearance_mm` 砍掉比指尖更近的殘留，並以 `surface_depth_window_mm` 排除背景。
- `SurfacePlane::fit` 會量測內點在平面上較窄方向的展開量，未達 `minimum_extent_mm` 即拒絕擬合；掃描中的 HUD 也會即時顯示這個數字。
- 表面掃描的樣本上限改為蓄水池取樣：上限約一秒就會填滿，舊行為會把之後掃過的區域全部丟棄，等於只用最初一瞬間的樣本決定平面。
